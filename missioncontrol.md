...
It runs a generic gateway that manages connections. It keeps the process alive, manages active sessions. It handles cron jobs (scheduled tasks). It routes messages between channels and sessions. It provides a WebSocket API for control.
...
clawdbot cron add \
   ...
   --cron "0,15,30,45 * * * *" \
   --session "isolated" \
   --message "You are Pepper, the Email Marketing Specialist. Check Mission Control..."
```
...
```bash
clawdbot sessions send --session "agent:seo-analyst:main" --message "Vision, can you..."
```
...
- Solving problems before users do
...
* Quill: 15 tweets drafted for approval
...
Writes hooks, threads, and LinkedIn posts. Knows what goes viral.
...
**Day 2:** Loki posts first draft. Status moves to Review. I review and give feedback. Loki revises. Done.
